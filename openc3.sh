...
case $1 in
  cli )
    ...
    docker run --rm -v `pwd`:/openc3/local:z -w /openc3/local $OPENC3_REGISTRY/openc3/openc3-enterprise-operator:$OPENC3_TAG ruby /openc3/bin/openc3cli $args
    set +a
    ;;
  cliroot )
    set -a
    . "$(dirname -- "$0")/.env"
    args=`echo $@ | { read _ args; echo $args; }`
    docker run --rm --user=root -v `pwd`:/openc3/local:z -w /openc3/local $OPENC3_REGISTRY/openc3/openc3-enterprise-operator:$OPENC3_TAG ruby /openc3/bin/openc3cli $args
    set +a
    ;;
  start )
    chmod -R 775 plugins
