...
case $1 in
  cli )
    ...
    docker run -it --rm --env-file "$(dirname -- "$0")/.env" --user=$OPENC3_USER_ID:$OPENC3_GROUP_ID -v `pwd`:/openc3/local:z -w /openc3/local $OPENC3_ENTERPRISE_REGISTRY/openc3/openc3-enterprise-operator:$OPENC3_ENTERPRISE_TAG ruby /openc3/bin/openc3cli $args
    set +a
    ;;
  cliroot )
    set -a
    . "$(dirname -- "$0")/.env"
    args=`echo $@ | { read _ args; echo $args; }`
    docker run -it --rm --env-file "$(dirname -- "$0")/.env" --user=root -v `pwd`:/openc3/local:z -w /openc3/local $OPENC3_ENTERPRISE_REGISTRY/openc3/openc3-enterprise-operator:$OPENC3_ENTERPRISE_TAG ruby /openc3/bin/openc3cli $args
    set +a
    ;;
  start )
  ...
  stop )
    ...
    ${DOCKER_COMPOSE_COMMAND} stop openc3-metrics
    sleep 5
