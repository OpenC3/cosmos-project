...
if "%1" == "cli" (
  ...
  docker run --rm --env-file %~dp0.env -v %cd%:/openc3/local -w /openc3/local openc3inc/openc3-operator:!OPENC3_TAG! ruby /openc3/bin/openc3cli !params!
  GOTO :EOF
)
if "%1" == "cliroot" (
  FOR /F "tokens=*" %%i in (%~dp0.env) do SET %%i
  set params=%*
  call set params=%%params:*%1=%%
  docker run --rm --env-file %~dp0.env --user=root -v %cd%:/openc3/local -w /openc3/local openc3inc/openc3-operator:!OPENC3_TAG! ruby /openc3/bin/openc3cli !params!
  GOTO :EOF
)
if "%1" == "start" (
  GOTO startup
